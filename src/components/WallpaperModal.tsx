...
export const WallpaperModal: React.FC<WallpaperModalProps> = ({
  ...
  const [customWallpapers, setCustomWallpapers] = useState<string[]>(() => {
    try {
      const saved = localStorage.getItem('xchord_custom_wallpapers');
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      ...
      if (dataUrl) {
        ...
        try {
          localStorage.setItem('xchord_custom_wallpapers', JSON.stringify(updated.slice(0, 10)));
        } catch {
          // ignore quota error
        }
  ...
  const handleRemoveCustomWp = (e: React.MouseEvent, wpUrl: string) => {
    ...
    setCustomWallpapers(updated);
    try {
      localStorage.setItem('xchord_custom_wallpapers', JSON.stringify(updated));
    } catch {
      // ignore
    }
